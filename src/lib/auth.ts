...
export const dataService = {
  // Appointments
  getAppointments: (userId: string, role: 'client' | 'lawyer'): Appointment[] => {
  ...
  getCases: (userId: string, role: 'client' | 'lawyer'): Case[] => {
    ...
    return cases.filter((case: Case) => 
      role === 'client' ? case.clientId === userId : case.lawyerId === userId
    );
  },

  createCase: (caseData: Omit<Case, 'id' | 'createdAt' | 'updatedAt' | 'documents' | 'tasks'>): Case => {
    const cases = JSON.parse(localStorage.getItem(STORAGE_KEYS.CASES) || '[]');
    const newCase: Case = {
      ...caseData,
      id: `case_${Date.now()}`,
  ...
  updateCase: (id: string, updates: Partial<Case>): void => {
    const cases = JSON.parse(localStorage.getItem(STORAGE_KEYS.CASES) || '[]');
    const index = cases.findIndex((case: Case) => case.id === id);
    if (index !== -1) {
...
export const initializeDemoData = () => {
  ...
  const existingAppointments = JSON.parse(localStorage.getItem(STORAGE_KEYS.APPOINTMENTS) || '[]');
  ...
  const existingCases = JSON.parse(localStorage.getItem(STORAGE_KEYS.CASES) || '[]');
    ...
    const demoCases: Case[] = [
      {
        ...
        description: 'Defense against criminal charges related to financial fraud allegations.',
        type: 'Criminal Law',
        status: 'active',
        priority: 'high',
        progress: 65,
    ...
    const demoTasks: Task[] = [
      {
        ...
        description: 'Please upload all financial documents related to the case including bank statements and tax returns.',
        assignedTo: '1', // client
        assignedBy: '2', // lawyer
    ...
    const demoMessages: ChatMessage[] = [
      {
        ...
        senderRole: 'lawyer',
        message: 'Hello John, I hope you\'re doing well. I\'ve reviewed your case details and we have a strong defense strategy.',
      ...
      },
      {
        ...
        senderRole: 'client',
        message: 'Thank you Sarah. I\'m feeling more confident about this. What documents do you need from me?',
      ...
      },
      {
        ...
        senderName: 'Sarah Chen',
        senderRole: 'lawyer',
        message: 'I\'ve assigned you a task to upload the financial documents. Please check your task list.',
